...
pub fn startServer(io: std.Io, gpa: std.mem.Allocator, config: Config) !void {
    ...
    // Without this, launchd fast-restart cycles cause the HTTP server to die silently.
    var listener: std.Io.net.Server = undefined;
    var bind_ok = false;
    for (0..30) |attempt| {
        listener = addr.listen(io, .{ .reuse_address = true }) catch |err| {
            std.debug.print("[host-http] bind port {d} failed (attempt {d}/30): {}\n", .{ config.port, attempt + 1, err });
    ...
    if (!bind_ok) {
        std.debug.print("[host-http] FATAL: could not bind port {d} after 30 attempts\n", .{config.port});
