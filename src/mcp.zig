...
fn handleVmStatus(gpa: std.mem.Allocator, io: std.Io, port: u16) ![]const u8 {
    ...
    const result = client.fetch(.{
        ...
        std.log.err("[mcp] vm_status HTTP error: {}", .{err});
        return error.StatusFailed;
    };

    if (result.status != .ok) {
        std.log.err("[mcp] vm_status HTTP {d}", .{@intFromEnum(result.status)});
        return error.StatusFailed;
    }
...
fn handleVmExec(gpa: std.mem.Allocator, io: std.Io, port: u16, vm: []const u8, command: []const u8) ![]const u8 {
    ...
    const escaped_cmd = try jsonEscape(gpa, command);
    defer gpa.free(escaped_cmd);
    const body = try std.fmt.allocPrint(gpa, "{{\"vm\":\"{s}\",\"command\":\"{s}\"}}", .{ vm, escaped_cmd });
    defer gpa.free(body);
    ...
    const uri = std.Uri.parse(url) catch |err| {
        std.log.err("[mcp] vm_exec bad URL: {}", .{err});
        return err;
    };
    ...
    var req = client.request(.POST, uri, .{
        ...
        std.log.err("[mcp] vm_exec HTTP error: {}", .{err});
        return error.ExecFailed;
    };
    defer req.deinit();

    req.sendBodyComplete(body) catch |err| {
        std.log.err("[mcp] vm_exec sendBody: {}", .{err});
        return error.ExecFailed;
    };

    var response = req.receiveHead(&redirect_buf) catch |err| {
        std.log.err("[mcp] vm_exec receiveHead: {}", .{err});
        return error.ExecFailed;
    };
    ...
    if (response.head.status != .ok) {
        ...
        std.log.err("[mcp] vm_exec HTTP {d}: {s}", .{ @intFromEnum(response.head.status), err_line });
        return error.ExecFailed;
    }
    ...
    while (true) {
        ...
        const n = body_reader.stream(&chunk_writer, std.Io.Limit.limited(4096)) catch |err| {
            std.log.err("[mcp] vm_exec read error: {}", .{err});
            break;
        };
        if (n == 0) break;
        try output.appendSlice(gpa, chunk_buf[0..n]);
    }
