...
| 45F | 版本 bump v0.18.83 + 文档（README/MANUAL/TOOLS_JSON/SKILL/DESIGN/host.zig 注释更正 Windows ConPTY→SSH_ASKPASS）+ 发布 | **✅ 完成**（2026-08-22）：代码+文档+bump 完成，门禁全绿（单测 230 + 集成 62 无泄漏）；用户批准发布 → commit 4870f57 → release.sh v0.18.83 → tag push → CI 全绿（unit+integration 测试、8 目标构建、Release 创建），GitHub Release v0.18.83 含 utmm.zip。**部署完成**：4 台 guest（linuxvm/macvm/windowsvm/winx64）经 `utmm --deploy` 全部成功推送 v0.18.83；本机 host 经 `--install --host` 重启至 v0.18.83；`--status` 5 节点全 serving。**经验**：手工 `cp` 替换 /opt/utmm/utmm 会触发 macOS taskgated SIGKILL（Code Signature Invalid）——必须 `codesign --force --sign -` 或走 svc selfCopy；deploy 优先走 serve-dir 缓存（`zig build cross` 产物需先拷入 /opt/utmm） | ✅ |
| 45G | 修复 45F 冒烟测试唯一失败项 + Windows Host sshpass 路径（v0.18.84） | **🔄 进行中**（2026-08-22）：用户确认"都修复" → Bug A：mcp.zig handleVmDownload 缺 flush+sync，Threaded Io 异步 close → 本地文件 0 字节，已修（参照 dpipe_file.zig copyAndDelete:219-220）；Bug B：handleVmSshpass 硬编码 /tmp（Windows Host 无 /tmp）→ 改用 svc.tempDir()+path.join，已修。门禁：单测 230 + 集成 62 无泄漏，test_mcp_tools.py **13/14 → 14/14**（download 内容 SHA 校验通过）。commit ad93aea。ver.txt → 0.18.84。**待办**：发布 + 部署 + Windows Host 切换补验 | 🔲 |
| 45H | Windows utmmd 反复崩溃 1067 根因修复：getAllIpsFingerprintWindows 栈踩踏 + panic 钩子 | **✅ 完成**（2026-08-22）：winx64/windowsvm UTM-MonitorD 反复 STOPPED(1067)，`--upgrade` 失败（utmmd 无法消费 tmp）、`--deploy` 成功。根因：GetAdaptersAddresses 第 4 参声明为 `addresses: *?*IP_ADAPTER_ADDRESSES_LH` 并传 `&addrs`（8 字节栈指针变量），API 按 size 往该地址写 15KB 结构数组 → 栈踩踏 → Debug 构建崩溃；monitorUtmm 首次 IP 检查（10s）触发，25.5s = stabilityCheck 10s + IP 检查 10s + SCM 重启 5.5s。修复：声明改 `addresses: ?*anyopaque` + `[16384]u8 align(8)` 缓冲区传 `&buf`；加 Windows panic 钩子（panic 消息写 utmmd-debug.log）。**实机验证**：windowsvm + winx64 部署后 UTM-MonitorD 均 RUNNING、utmmd PID 稳定（>60s 无重启）、无 PANIC、mesh/exec 通道恢复；门禁单测 230 + 集成 62 无泄漏全绿。**后续**：回到 v0.18.85 起连续 bump 验证 --upgrade 流畅性 | ✅ |
