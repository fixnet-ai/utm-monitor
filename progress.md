...
关键转折：给 utmmd 致死路径补事件行后，winx64 日志一句
`heartbeat timeout (hb=384440237 now=384450894 age=10657ms)` 直接指认冻结的心跳，
随即在 tcp.zig 找到 Windows 分支漏设 FIONBIO——**探针先行，一次定位**。
另发现 deploy Windows 分支绕过 --install 导致 utmmd 六天未更新（utmm/utmmd 版本漂移）。

### 验证记录

- v0.18.75: 探针版，winx64 手动 --install 激活新 utmmd 后立即抓到死因
- v0.18.76: 双修复版，deploy 4/4（新安装路径），winx64/windowsvm PID 稳定、日志静默
- 门禁：221 单测 + 60 集成 + 4 目标交叉编译（含 3 Windows）全过
