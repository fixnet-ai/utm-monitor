...
fn netBindIpImpl(_: ?*anyopaque, address: *const Io.net.IpAddress, options: Io.net.IpAddress.BindOptions) Io.net.IpAddress.BindError!Io.net.Socket {
    ...
    // Always set SO_REUSEADDR for UDP datagram sockets so multiple
    // listeners can share the same port (e.g. Host daemon + --status).
    // On macOS/BSD, SO_REUSEPORT is also required for true port sharing.
    if (options.mode == .dgram) {
        const value: c_int = 1;
        os_net.setsockopt(handle, os_net.SOL.SOCKET, os_net.SO.REUSEADDR, std.mem.asBytes(&value)) catch {};
        if (@hasDecl(os_posix.SO, "REUSEPORT")) {
            os_net.setsockopt(handle, os_net.SOL.SOCKET, os_posix.SO.REUSEPORT, std.mem.asBytes(&value)) catch {};
        } else if (builtin.os.tag != .windows) {
            // macOS/BSD: SO_REUSEPORT = 0x0200 (not declared in Zig 0.16.0 posix)
            os_net.setsockopt(handle, os_net.SOL.SOCKET, 0x0200, std.mem.asBytes(&value)) catch {};
        }
    }
